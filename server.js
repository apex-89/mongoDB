...
const MyVegetable = require('./models/vegetables')
...
let connectionString = `mongodb+srv://${process.env.MONGOUSERNAME}:${process.env.MONGOPASSWORD}@mongosetupcluster.1eeko3p.mongodb.net/fruits?retryWrites=true&w=majority`;
...
app.put('/update_fruit', async (req,res) => {
...
app.get('/fruit/:fruit', async (req,res) => {
    let id = req.params.fruit;
    let response = await MyFruit.findById(id)
    res.send(response)
})

app.get('/veggie/:veggie', async (req,res) => {
    let id = req.params.veggie;
    let response = await MyVegetable.findById(id)
    res.send(response)
})

app.get('/veggies', async (req,res) => {
    let response = await MyVegetable.find({});
    res.json(response)
})
